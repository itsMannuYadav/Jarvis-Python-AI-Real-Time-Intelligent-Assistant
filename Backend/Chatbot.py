...
def Realtimelnformation():
    ...
    def ChatBot(Query):
        ...
        try:
            ...
            # Append the user's query to the
            messages.append({"role": "user", "content": f"{Query}"})   
    
            # Ma/<e a request to the Groq API for a response.
            completion

            # Append the user's query to the messages list.
            messages.append({"role": "user", "content" : f"{Query}"})

            # Make request to the Groq API for a response.
            completion = client.chat.completions.create(
                model = "llama3-70b-8192" # Specifv the AI model to use.
                messages = SystemChatBot + [{"role": "system", "content" : Realtimelnformation()}] + messages, # Include system instructions, real-time info, a
                max_tokens = 1024,  # Limit the maximum tokens in the response.
                temperature = 0.7, # Adjust response randomness (higher means more random).           
                top_p = l, # Use nucleus sampling to control diversity
            ...
            Answer = Answer.replace("</s>","")  # Clean up anv unwanted tokens from the response.
            
            # Append the chatbot's response to the message list.
            messages.append({"rote": "assistant"," content": Answer})
